...
export default function ManageGarden() {
    ...
    const [gardenManagers, setGardenManagers] = useState(null);
    ...
    const getGardenManagers = () => {
      const config = {
        headers: { Authorization: `Bearer ${token}` }
      };

      axios.get(baseUrl + '/users/garden_manager', config).then((response) => {
        setGardenManagers(response.data.data);
      }).catch((error) => {
    ...
    const registerGardenManager = () => {
        ...
        axios.post(baseUrl + '/register', payload).then((response) => {
            handleCancel();
            getGardenManagers();
            toast.success("Registered Garden Manager Successfully!");
        }).catch((error) => {
    ...
    const handleUpdate = (event) => {
        ...
        .then((response) => {
            toast.success("Garden Manager Details updated Successfully!");
            handleCancel();
            getGardenManagers();
        }).catch((error) => {
            toast.error(error);
    ...
    const handleAction = (actionData) => {
        if(actionData.action === 'edit') {
            let editData = gardenManagers.find(o => o.id === actionData.id);
            setEditId(actionData.id);
            handleEdit(editData);
        }
        else if(actionData.action === 'status') {
            let statusData = gardenManagers.find(o => o.id === actionData.id);
            ...
            <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
                <div className="rounded-t bg-white mb-0 px-6 py-6">
                    <div className="text-center flex justify-between">
                        <h6 className="text-blueGray-700 text-xl font-bold">Manage Garden Managers</h6>
                    </div>
                </div>
                ...
                <ManagersList
                    title="Garden Managers"
                    managers={gardenManagers}
