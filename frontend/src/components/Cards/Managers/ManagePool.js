...
export default function ManagePool() {
    ...
    const [poolManagers, setPoolManagers] = useState(null);
    ...
    const getPoolManagers = () => {
      const config = {
        headers: { Authorization: `Bearer ${token}` }
      };

      axios.get(baseUrl + '/users/pool_manager', config).then((response) => {
        setPoolManagers(response.data.data);
      }).catch((error) => {
    ...
    const registerPoolManager = () => {
        ...
        axios.post(baseUrl + '/register', payload).then((response) => {
            handleCancel();
            getPoolManagers();
            toast.success("Registered Pool Manager Successfully!");
        }).catch((error) => {
    ...
    const handleUpdate = () => {
        ...
        .then((response) => {
            toast.success("Pool Manager Details updated Successfully!");
            handleCancel();
            getPoolManagers();
        }).catch((error) => {
            toast.error(error);
    ...
    const handleAction = (actionData) => {
        if(actionData.action === 'edit') {
            let editData = poolManagers.find(o => o.id === actionData.id);
            setEditId(actionData.id);
            handleEdit(editData);
        }
        else if(actionData.action === 'status') {
            let statusData = poolManagers.find(o => o.id === actionData.id);
            ...
            <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
                <div className="rounded-t bg-white mb-0 px-6 py-6">
                    <div className="text-center flex justify-between">
                        <h6 className="text-blueGray-700 text-xl font-bold">Manage Pool Managers</h6>
                    </div>
                </div>
