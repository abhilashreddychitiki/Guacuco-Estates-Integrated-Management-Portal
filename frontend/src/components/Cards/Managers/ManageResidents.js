...
export default function ManageResident() {
    ...
    const getResident = () => {
      ...
      axios.get(baseUrl + '/users/resident', config).then((response) => {
    ...
    const registerResident = () => {
        ...
        axios.post(baseUrl + '/register', payload).then((response) => {
            handleCancel();
            getResident();
            toast.success("Registered Resident Successfully!");
        }).catch((error) => {
    ...
    const handleAction = (actionData) => {
        if(actionData.action === 'edit') {
            let editData = resident.find(o => o.id === actionData.id);
            setEditId(actionData.id);
            handleEdit(editData);
        }
        else if(actionData.action === 'status') {
            let statusData = resident.find(o => o.id === actionData.id);
            ...
            <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
                <div className="rounded-t bg-white mb-0 px-6 py-6">
                    <div className="text-center flex justify-between">
                        <h6 className="text-blueGray-700 text-xl font-bold">Manage Residents</h6>
                    </div>
                </div>
